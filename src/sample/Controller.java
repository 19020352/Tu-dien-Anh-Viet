...
public class Controller implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        TextFields.bindAutoCompletion(searchBar,( t -> Database.d.target.stream().filter(elem ->
                elem.toLowerCase().startsWith(t.getUserText().toLowerCase())).limit(18)
                .collect(Collectors.toList())));
    }

    public void audioButtonClicked(ActionEvent event) throws Exception  {
        if (legitWord) {
            ...
            if( HttpURLConnection.HTTP_OK == responseCode) {
                ...
                alert.setHeaderText("Chưa có phát âm của từ này!");
                alert.setGraphic(new ImageView(this.getClass().getResource("sorry.png").toString()));
                alert.showAndWait();}
        }
    }
}
